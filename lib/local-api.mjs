 ...
 *
 * 支持两种 AI 评分引擎（教师后台可切换）：
 *  - heuristic: 本地启发式评分（默认，离线可用）
 *  - deepseek: 通过 Cloudflare Worker 代理调用 DeepSeek API（API key 只存在后端）
 *
 * API key 安全：前端永远不持有 DeepSeek API key。
 * Worker URL 配置优先级：
 *  1. localStorage 中教师后台配置的 proxyUrl
 *  2. 内置默认值（部署后替换为实际 Worker URL）
 */
...
function assertSubmission(body) {
  ...
  if (
    ...
    payload.answers.some((a) => typeof a !== "string" || !a)
  ) {
...
/**
 * 读取教师后台配置的 AI 引擎和 DeepSeek API key。
 */
...
/**
 * 运行 AI 分析，支持 deepseek / heuristic 两种引擎。
 * 失败时自动回退到 heuristic，保证永远有结果。
 */
...
export async function handleLocalApi(method, path, body = {}, headers = {}) {
  ...
  if (method === "POST" && path === "/submit") {
    ...
    try {
      ...
      validateAnswerIds(payload.answers, questionList);
    } catch (err) {
